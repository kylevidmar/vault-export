...
Export your Hashicorp Vault to a file. Not guaranteed to be consistent.
Export is a form of commands to inject keys into vault, so it is convenient to
...
The following environment variables are used:
 ...
 - TOP_VAULT_PREFIX - path to export, optional, useful when exporting only a fraction of the strucutre of the vault
...
After succesful authorization you can run export script and encrypt it with gpg to the output file:

```bash
python vault-export.py | gpg -e -r GPG_KEY_ID > vault.export.txt.gpg
